...
Skeletons are widely used in biomedical image analysis. They are a stick figure that can represent the geometric or topological structure of biological strucutres such as neurons, blood vessels and aorta, bones, gastrointestinal lumen, and more. Skeletons (sometimes called medial paths or a medial axis transform) may be represented as a possibly cyclic undirected graph over vertices that are fixed in 2d or 3d space. The vertices and edges may additionally be annotated with e.g. radius to the nearest border, cross sectional area, or other measurements.
